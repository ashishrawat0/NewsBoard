...
export const SourceProvider = (props) => {
  ...
  useEffect(() => {
    const fetchChannel = async () => {
      const res = await axios.get(
        'https://newsapi.org/v2/sources?apiKey=fc62c650d19b4e3186cad1cc396ef847'
      );
  ...
  const currentChannel = channels.slice(
    indexofFirstChannel,
    indexOfLastChannel
  );
  return (
    <SourceContext.Provider
      value={{ value: [channels, setChannel], value1: [currentChannel] }}
    >
      {props.children}
    </SourceContext.Provider>
  );
